...
coinOptions.forEach(option => {
    option.addEventListener('click', function () {
        if (this.id !== 'custom-option') {
            ...
            // buyButton.textContent = `US$ ${price}`;
        }
    });
});
...
buyButton.addEventListener('click', function () {
    ...
    coinsNum_1.textContent = coins;
    coinsPrice_1.innerHTML = "US$" + price;
    coinsPrice_2.innerHTML = "US$" + price;
...
function updateInput() {
    ...
    numValue = parseFloat(numValue / 94.33);


    totalAmount.textContent = `US$${numValue.toLocaleString()}`;
